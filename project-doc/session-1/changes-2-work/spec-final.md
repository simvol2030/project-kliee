...
**Путь:** `/shop/collections` или `/admin/works` (уточнить по существующей структуре)
...
## Загрузка изображений — требования к реализации

### Проблема дублирования

При загрузке изображений через админку необходимо исключить дублирование:

1. **Проверка перед загрузкой:**
   - Вычислять хэш файла (MD5 или SHA256) перед сохранением
   - Проверять наличие файла с таким хэшем в БД
   - Если файл существует — использовать существующую запись, не создавать дубликат

2. **Структура хранения:**
   - Изображения коллекций: `/uploads/series/` или `/images/works/` (для совместимости)
   - Изображения работ: `/uploads/artworks/`
   - Уникальное имя файла: `{timestamp}_{originalName}` или `{hash}.{ext}`

3. **Таблица media:**
   - Добавить поле `file_hash` для проверки дубликатов
   - При загрузке: проверить хэш → если есть — вернуть существующий ID
   - Связь через `artworkImages` остаётся (artwork_id → media_id)

4. **UI загрузки в админке:**
   - Drag & drop для множественной загрузки
   - Превью перед сохранением
   - Прогресс-бар загрузки
   - Сообщение если файл уже существует ("Изображение уже загружено, используется существующее")
   - Возможность удалить связь изображения с работой (не удаляя сам файл, если используется в другом месте)

5. **Удаление изображений:**
   - При удалении связи artwork ↔ image — проверить, используется ли media в других местах
   - Если media не используется нигде — удалить файл с диска
   - Если используется — только удалить связь в `artworkImages`

### Совместимость путей

- Существующие изображения: `/images/works/...` — НЕ трогать
- Новые загрузки: `/uploads/...`
- В БД хранить относительный путь, на фронтенде корректно резолвить оба варианта
...
- **Без дублирования** — хэширование файлов, проверка перед сохранением
...
- [ ] Изображения: загрузка без дублирования (проверка по хэшу)
- [ ] Изображения: корректное удаление (с проверкой использования)
...
**Загрузка изображений (антидублирование):**
- [ ] Загрузить новое изображение — сохраняется корректно
- [ ] Загрузить то же изображение повторно — НЕ создаётся дубликат
- [ ] Появляется сообщение "Изображение уже существует"
- [ ] Удалить изображение у работы — файл НЕ удаляется если используется в другом месте
- [ ] Удалить изображение которое нигде больше не используется — файл удаляется с диска
...
| Файл | Изменение |
|------|-----------|
| `src/lib/server/db/schema.ts` | Проверить/добавить поля series, добавить file_hash в media |
| `src/lib/data/series.provider.ts` | Переключить с JSON на БД |
| `data/series.json` | Источник для миграции → потом оставить как бэкап |
...
| `src/lib/server/upload.ts` | Добавить хэширование и проверку дубликатов |
